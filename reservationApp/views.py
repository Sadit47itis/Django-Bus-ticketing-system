...
# Custom decorator to check if user is a superuser
...
def scheduled_trips(request):
    if not request.method == 'POST':
        ...
        schedules = Schedule.objects.filter(Q(status = 1) & Q(Q(depart = depart) & Q(destination = destination ))).all()
        #schedules = Schedule.objects.filter(Q(status = 1) & Q(schedule__month = month) & Q(schedule__day = day) & Q(schedule__year = year) & Q(Q(depart = depart) | Q(destination = destination ))).all()
        context['schedules'] = schedules
